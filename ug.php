...
Plugin URI: http://doghap.net/wp-bfont_1-5.html
...
add_action('admin_init','fontfaces_css');
add_action('wp_enqueue_scripts', 'remove_fontFaces');
add_action('login_enqueue_scripts', 'fontfaces_css');
...
function fontfaces_css () {
	if(esc_attr( get_the_author_meta( 'user_font_family', get_current_user_ID() ) ) == 'UKIJ Tuz Tom'){
		wp_enqueue_style('ug_vk', plugins_url('includes/font2.css', __FILE__));
	}else{
		wp_enqueue_style('ug_vk', plugins_url('includes/font.css', __FILE__));
	}
}
